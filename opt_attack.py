...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, args, dist_test=False, save_to_file=False,
                   result_dir=None):
    ...
    if key == 'voxels':
        max_num_points_per_voxel = [x['MAX_POINTS_PER_VOXEL'] for x in cfg.DATA_CONFIG.DATA_PROCESSOR if
                                    x['NAME'] == 'transform_points_to_voxels'][0]
        max_num_voxels = [x['MAX_NUMBER_OF_VOXELS'] for x in cfg.DATA_CONFIG.DATA_PROCESSOR if
                          x['NAME'] == 'transform_points_to_voxels'][0]['test']
        voxel_size = \
        [x['VOXEL_SIZE'] for x in cfg.DATA_CONFIG.DATA_PROCESSOR if x['NAME'] == 'transform_points_to_voxels'][0]
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        if rand_init: # Initialize the perturbation randomly at the start (PGD)
            ...
            if key == 'voxels':
                ...
                batch_dict['voxels'] = torch.from_numpy(voxels[:, :, :num_point_features]).float().cuda(
                    key_origin.device)
                ...
                batch_dict[key][:, 1:5] = batch_dict[key][:, 1:5] + perturbation
            ...
            if key == 'voxels':
                ...
                voxel_points_index = torch.zeros(batch_dict['voxels'].shape[0] * batch_dict['voxels'].shape[1],
                                                 device=key_origin.device)
                valid_points_index = torch.arange(len(key_origin), device=key_origin.device)
                try:
                    voxel_points_index[torch.sum(batch_dict['voxels'].abs(),
                                                 dim=2).flatten().nonzero().flatten()] = valid_points_index.float()
                except:
                    import pdb;
                    pdb.set_trace()
                voxels = torch.cat([batch_dict[key], voxel_points_index.view(batch_dict['voxels'].shape[0],
        ...
        for i in range(nb_iter):
            ...
            loss = focal_loss(logits, targets_one_hot, weights)
            ...
            for k in ['batch_index', 'point_cls_scores', 'batch_cls_preds', 'batch_box_preds', 'cls_preds_normalized',
                      'rois', 'roi_scores', 'roi_labels', 'has_class_labels']:
                batch_dict.pop(k, None)  # adhoc

            for k in ['voxel_features', 'encoded_spconv_tensor', 'encoded_spconv_tensor_stride',
            ...
            if (clip_min is not None) or (clip_max is not None):
                ...
                #batch_dict['points'][:, 1:5] = torch.clamp(batch_dict['points'][:, 1:5], clip_min, clip_max) # intensity
        ...
        for j, pred_dict in enumerate(pred_dicts):
            ...
            num_modified += np.sum(original_non_cars & predicted_as_car)

        logger.info(f'Number of non-"Car" bounding boxes changed to "Car": {num_modified}')
